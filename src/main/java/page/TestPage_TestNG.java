...
public class TestPage_TestNG {
	...
	public TestPage_TestNG(WebDriver driver) {
		this.driver = driver;
	}
	...
	public void validateCategoryCreated() {
		...
		Assert.assertEquals(lastCategoryName, testCategoryName, "Category not created.");
	...
	public void validateMonths() {
		...
		for(int i=0; i<=expectedMonths.size()-1; i++) {
			...
			try {
			System.out.println(" Expected: " + actualMonths.get(i));
